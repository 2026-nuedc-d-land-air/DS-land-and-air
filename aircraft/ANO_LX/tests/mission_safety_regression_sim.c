...
static int scenario_drop_complete(void)
{
    ...
    expect(dis_x_target == 134 && dis_y_target == -62,
           "DROP_ALIGN did not apply the body +X and +Y release compensation");
...
static int scenario_dynamic_lead_and_fixed_yaw(void)
{
    setup_healthy_inputs();
    start_task(LX_TASK_DYNAMIC_LAND, 22u);
    run_to_stage(MISSION_STAGE_FOLLOW, 1600u);
    tick_perfect_tracking(1u);
    expect(dis_x_target == 136 && dis_y_target == -70,
           "dynamic tracking did not apply car lead and lidar mounting compensation");
    expect(yaw_target == 0,
           "dynamic tracking followed the car yaw instead of its latched yaw");
    puts("[PASS] dynamic_lead_and_fixed_yaw");
    return 0;
}

static int scenario_dynamic_descent_velocity(void)
{
    setup_healthy_inputs();
    start_task(LX_TASK_DYNAMIC_LAND, 23u);
    run_to_stage(MISSION_STAGE_DESCEND, 2000u);
    tick_perfect_tracking(1u);
    expect(height_target == 10,
           "dynamic descent did not command the 10 cm contact target directly");
    expect(s_height_velocity_limit == 55,
           "dynamic descent did not enable the configured 55 cm/s height velocity limit");
    puts("[PASS] dynamic_descent_velocity");
    return 0;
}

static int scenario_dynamic_final_descent_commits(void)
{
    int32_t final_x;
    int32_t final_y;

    setup_healthy_inputs();
    start_task(LX_TASK_DYNAMIC_LAND, 24u);
    run_to_stage(MISSION_STAGE_DESCEND, 2000u);
    tick_perfect_tracking(1u);
    final_x = dis_x_target;
    final_y = dis_y_target;
    tick_perfect_tracking_xy(300, 300);
    expect(height_target == 10,
           "final dynamic descent stopped commanding the contact height");
    expect(dis_x_target == final_x && dis_y_target == final_y,
           "final dynamic descent resumed chasing the moving platform");
    puts("[PASS] dynamic_final_descent_commits");
    return 0;
}
...
static int scenario_platform_remains_armed(void)
{
    ...
    expect(height_target == 10,
           "platform contact did not apply the configured 10 cm contact target");
    ...
    expect(keep_hight_flag == 0 && keep_radar_flag == 0 && keep_yaw_flag == 0,
           "platform contact did not release height, position, and yaw control");
    expect(s_height_velocity_limit == 20,
           "platform contact did not restore the normal height velocity limit");
    ...
    expect(keep_hight_flag != 0 && keep_radar_flag != 0 && keep_yaw_flag != 0,
           "platform takeoff did not restore flight controls");
    expect(s_height_velocity_limit == 50,
           "platform takeoff did not enable the 50 cm/s height velocity limit");
    takeoff_x = dis_x_target;
    takeoff_y = dis_y_target;
    tick_perfect_tracking_xy(300, 300);
    expect(dis_x_target == takeoff_x && dis_y_target == takeoff_y,
           "platform takeoff continued following the moving car instead of climbing in place");
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "dynamic_lead_and_fixed_yaw") == 0)
        return scenario_dynamic_lead_and_fixed_yaw();
    if (strcmp(argv[1], "dynamic_descent_velocity") == 0)
        return scenario_dynamic_descent_velocity();
    if (strcmp(argv[1], "dynamic_final_descent_commits") == 0)
        return scenario_dynamic_final_descent_commits();
