...
#define DYNAMIC_PLATFORM_CONTACT_RANGE_CM  10
#define DYNAMIC_FINAL_DESCENT_COMMIT_RANGE_CM 60
#define DYNAMIC_CAR_BODY_X_LEAD_CM         20
#define DYNAMIC_LIDAR_BODY_Y_MOUNT_OFFSET_CM (-16)
#define DEFAULT_HEIGHT_VELOCITY_LIMIT_CMPS 20
#define DYNAMIC_DESCENT_HEIGHT_VELOCITY_LIMIT_CMPS 45
#define DYNAMIC_PLATFORM_TAKEOFF_HEIGHT_VELOCITY_LIMIT_CMPS 50
#define DROP_PLATFORM_RANGE_CM             65
#define DROP_RELEASE_BODY_X_ADVANCE_CM     12
#define DROP_RELEASE_BODY_Y_MOUNT_OFFSET_CM (-14)
...
#define DYNAMIC_PLATFORM_CONTACT_DETECT_RANGE_CM LANDING_TERMINAL_RANGE_CM
...
/* Internal alignment blockers used by the terminal controller.  These are
 * not serialized; keep them local to the flight-control implementation. */
#define ALIGN_BLOCK_CAR_POSE              0x01u
#define ALIGN_BLOCK_CAMERA_LINK           0x02u
#define ALIGN_BLOCK_POSITION              0x04u
#define ALIGN_BLOCK_HEIGHT                0x08u
#define ALIGN_BLOCK_YAW                   0x10u
...
static int32_t s_platform_takeoff_x_cm;
static int32_t s_platform_takeoff_y_cm;
static int32_t s_final_descent_x_cm;
static int32_t s_final_descent_y_cm;
static uint8_t s_final_descent_target_valid;
static int32_t s_range_target_cm;
static int32_t s_dynamic_yaw_hold_deg;
...
static int32_t task_yaw_hold_deg(void)
{
    return (s_task_type == LX_TASK_DYNAMIC_LAND)
               ? s_dynamic_yaw_hold_deg
               : car_yaw_deg();
}
...
static void car_target(uint32_t now, uint8_t use_visual, int32_t *target_x, int32_t *target_y)
{
    ...
    *target_y = car_y;
    ...
    if (s_task_type == LX_TASK_DYNAMIC_LAND)
    {
        double yaw_rad = ((double)s_pose.yaw_0p1deg * 0.1) * (double)RAD_PER_DEG;
        double body_x = (double)DYNAMIC_CAR_BODY_X_LEAD_CM;
        double body_y = -(double)DYNAMIC_LIDAR_BODY_Y_MOUNT_OFFSET_CM;
        int32_t lead_x = round_f64_to_i32(body_x * (double)my_cos(yaw_rad) -
                                          body_y * my_sin(yaw_rad));
        int32_t lead_y = round_f64_to_i32(body_x * my_sin(yaw_rad) +
                                          body_y * (double)my_cos(yaw_rad));

        /* Lead the moving platform along its own forward (+X) direction.
         * The lidar is mounted on body -Y, so move the aircraft center +Y
         * to put the lidar footprint over the platform center. */
        *target_x += lead_x;
        *target_y += lead_y;
    }
...
static void release_platform_contact_controls(void)
{
    /* Keep the FC armed at idle while sitting on the platform. */
    pid_set_height_velocity_limit(DEFAULT_HEIGHT_VELOCITY_LIMIT_CMPS);
    keep_hight_flag = 0;
    keep_radar_flag = 0;
    keep_yaw_flag = 0;
    pid_set_radar_xy_limit(0);
}

static void apply_drop_release_body_offset(int32_t *target_x, int32_t *target_y)
{
    double yaw_rad = ((double)yaw_slam * 0.1) * (double)RAD_PER_DEG;
    double body_x = (double)DROP_RELEASE_BODY_X_ADVANCE_CM;
    double body_y = -(double)DROP_RELEASE_BODY_Y_MOUNT_OFFSET_CM;
    int32_t world_x = round_f64_to_i32(body_x * (double)my_cos(yaw_rad) -
                                       body_y * my_sin(yaw_rad));
    int32_t world_y = round_f64_to_i32(body_x * my_sin(yaw_rad) +
                                       body_y * (double)my_cos(yaw_rad));

    /* The released object lands about 15 cm toward body -X, so pre-position +X.
     * The release mechanism is mounted 14 cm on body -Y, so pre-position +Y. */
...
static void accept_request(const lx_mission_request_t *request)
{
    ...
    s_platform_takeoff_x_cm = s_home_x_cm;
    s_platform_takeoff_y_cm = s_home_y_cm;
    s_final_descent_x_cm = s_pose.x_cm;
    s_final_descent_y_cm = s_pose.y_cm;
    s_final_descent_target_valid = 0u;
    s_range_target_cm = GROUND_CRUISE_RANGE_CM;
    s_dynamic_yaw_hold_deg = round_f64_to_i32((double)yaw_slam * 0.1);
    s_visual_filter_initialized = 0u;
...
void UserTask_OneKeyCmd(void)
{
    ...
    case MISSION_STAGE_TAKEOFF:
        ...
        yaw_set_hold_target(task_yaw_hold_deg());
        set_radar_target(s_home_x_cm, s_home_y_cm,
    ...
    case MISSION_STAGE_LAND_ALIGN:
        ...
        {
            apply_drop_release_body_offset(&target_x, &target_y);
        }
                           ...
                           : LAND_ALIGN_STABLE_MS) != 0u)
        {
            ...
            {
                s_range_target_cm = PLATFORM_CRUISE_RANGE_CM;
                s_final_descent_x_cm = target_x;
                s_final_descent_y_cm = target_y;
                s_final_descent_target_valid = 1u;
                set_stage(MISSION_STAGE_DESCEND);
                /* Commit the descent from this verified alignment point so
                 * the aircraft does not keep chasing the moving platform. */
                s_stage_substate = 1u;
            }
        }
    ...
    case MISSION_STAGE_DESCEND:
        ...
        if (s_stage_substate == 1u)
        {
            /* Descent is deliberately committed: do not chase a moving
             * platform after leaving the verified alignment point. */
            target_x = s_final_descent_x_cm;
            target_y = s_final_descent_y_cm;
            s_descent_inhibited = 0u;
            s_range_target_cm = DYNAMIC_PLATFORM_CONTACT_RANGE_CM;
            pid_set_height_velocity_limit(DYNAMIC_DESCENT_HEIGHT_VELOCITY_LIMIT_CMPS);
            set_radar_target(target_x, target_y, s_range_target_cm, TERMINAL_SPEED_CMPS);
            if (stable_for((uint8_t)(hight <= DYNAMIC_PLATFORM_CONTACT_DETECT_RANGE_CM),
                           LAND_CONTACT_STABLE_MS) != 0u)
            {
                release_platform_contact_controls();
        ...
        if ((uint32_t)(now - s_pose.local_rx_ms) > CAR_POSE_WARNING_MS)
        {
            ...
            s_range_target_cm = clamp_i32(hight,
                                          DYNAMIC_PLATFORM_CONTACT_RANGE_CM,
                                          PLATFORM_CRUISE_RANGE_CM);
            s_descent_step_ms = 0u;
        }
        ...
        else
        {
            ...
            if (aligned != 0u)
            {
                /* Once horizontal alignment is stable, command the platform
                 * contact height directly.  The FC height loop then applies
                 * its configured descent velocity instead of a 1 cm/100 ms
                 * software gradient. */
                s_range_target_cm = DYNAMIC_PLATFORM_CONTACT_RANGE_CM;
                pid_set_height_velocity_limit(DYNAMIC_DESCENT_HEIGHT_VELOCITY_LIMIT_CMPS);
                s_final_descent_x_cm = target_x;
            ...
            {
                /* Do not climb back to the 135 cm approach height when the
                 * moving-platform target briefly exceeds the XY gate. */
                s_range_target_cm = clamp_i32(hight,
                                              DYNAMIC_PLATFORM_CONTACT_RANGE_CM,
                                              PLATFORM_CRUISE_RANGE_CM);
                pid_set_height_velocity_limit(DEFAULT_HEIGHT_VELOCITY_LIMIT_CMPS);
        ...
        if (hight <= DYNAMIC_FINAL_DESCENT_COMMIT_RANGE_CM &&
            s_final_descent_target_valid != 0u)
        {
            s_stage_substate = 1u;
            target_x = s_final_descent_x_cm;
        ...
        set_radar_target(target_x, target_y, s_range_target_cm, TERMINAL_SPEED_CMPS);
        if (stable_for((uint8_t)((s_stage_substate != 0u || aligned != 0u) &&
                                 hight <= DYNAMIC_PLATFORM_CONTACT_DETECT_RANGE_CM),
                       LAND_CONTACT_STABLE_MS) != 0u)
        {
    ...
    case MISSION_STAGE_ON_PLATFORM_5S:
        ...
        if (stable_for((uint8_t)(hight <= (DYNAMIC_PLATFORM_CONTACT_DETECT_RANGE_CM + 5) &&
    ...
    case MISSION_STAGE_PLATFORM_TAKEOFF:
        if (s_stage_substate == 0u)
        {
            ...
            mode_select(4);
            /* The platform departure needs enough positive vertical authority
             * to clear it before lateral disturbance can develop. */
            pid_set_height_velocity_limit(DYNAMIC_PLATFORM_TAKEOFF_HEIGHT_VELOCITY_LIMIT_CMPS);
            s_dynamic_yaw_hold_deg = round_f64_to_i32((double)yaw_slam * 0.1);
            get_air_xy(&s_platform_takeoff_x_cm, &s_platform_takeoff_y_cm);
            s_platform_takeoff_x_cm = clamp_i32(s_platform_takeoff_x_cm,
                                                FIELD_X_MIN_CM, FIELD_X_MAX_CM);
            s_platform_takeoff_y_cm = clamp_i32(s_platform_takeoff_y_cm,
                                                FIELD_Y_MIN_CM, FIELD_Y_MAX_CM);
            s_stage_substate = 2u;
            s_range_target_cm = PLATFORM_CRUISE_RANGE_CM;
        }
        ...
        {
            yaw_set_hold_target(task_yaw_hold_deg());
            set_radar_target(s_platform_takeoff_x_cm, s_platform_takeoff_y_cm,
                             PLATFORM_CRUISE_RANGE_CM, FOLLOW_SPEED_CMPS);
            if (stable_for((uint8_t)(position_arrived(s_platform_takeoff_x_cm,
                                                       s_platform_takeoff_y_cm,
                                                       FOLLOW_ARRIVE_CM) &&
                                     height_arrived(PLATFORM_CRUISE_RANGE_CM) &&
                                     yaw_arrived(YAW_ARRIVE_DEG)),
